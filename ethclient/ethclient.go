...
// SideBalanceAt returns the wei balance of the given account.
// The block number can be nil, in which case the balance is taken from the latest known block.
func (ec *Client) SideBalanceAt(ctx context.Context, appId string, account common.Address, blockNumber *big.Int) (*big.Int, error) {
	var result hexutil.Big
	err := ec.c.CallContext(ctx, &result, "eth_getSideBalance", appId, account, toBlockNumArg(blockNumber))
	return (*big.Int)(&result), err
}
