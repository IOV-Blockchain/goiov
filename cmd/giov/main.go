...
// giov is the official command-line client for GIOV.
package main
...
const (
	clientIdentifier = "giov" // Client identifier to advertise over the network
)
...
var (
	...
	app = utils.NewApp(gitCommit, "the giov command line interface")
...
func init() {
	// Initialize the CLI app and start Giov
	app.Action = giov
	app.HideVersion = true // we have a command to print the version
	app.Copyright = "Copyright 2018 The CarLiveChain Authors"
...
// giov is the main entry point into the system if no special subcommand is ran.
// It creates a default node based on the command line arguments and runs it in
// blocking mode, waiting for it to be shut down.
func giov(ctx *cli.Context) error {
...
func startNode(ctx *cli.Context, stack *node.Node) {
	...
	if ctx.GlobalBool(utils.MiningEnabledFlag.Name) || ctx.GlobalBool(utils.DeveloperFlag.Name) {
		...
		ethereum.BlockChain().Config().Alien.UnMine = false
