...
import (
	...
	"github.com/CarLiveChainCo/goiov/common/fdlimit"
...
var (
	...
	TestnetFlag = cli.BoolFlag{
		Name:  "testnet",
		Usage: "IOV test network: pre-configured delegate-proof-of-stake test network",
	}
	TestgiovFlag = cli.BoolFlag{
		Name:  "testgiov",
		Usage: "IOV test network: pre-configured delegate-proof-of-stake test network",
	}
...
func setBootstrapNodes(ctx *cli.Context, cfg *p2p.Config) {
	...
	case ctx.GlobalBool(TestgiovFlag.Name):
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *eth.Config) {
	...
	case ctx.GlobalBool(TestnetFlag.Name):
		...
		appmanager.RealAppManagerAddress = appmanager.TestNetAppManagerAddress
...
func RegisterEthService(stack *node.Node, cfg *eth.Config, nodeCfg *node.Config) {
	var err error
	if cfg.SyncMode == downloader.LightSync {
		...
		err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			fullNode, err := eth.New(ctx, cfg, nodeCfg)
