...
func ReadCanonicalHash(db DatabaseReader, number uint64, appid ...string) common.Hash {
	var key = append(append(headerPrefix, encodeBlockNumber(number)...), headerHashSuffix...)
...
func ReadHeaderNumber(db DatabaseReader, hash common.Hash, appid ...string) *uint64 {
	var key = append(headerNumberPrefix, hash.Bytes()...)
	if len(appid) == 1 && appid[0] != "" {
		key = append([]byte(appid[0]), key...)
	}
	data, _ := db.Get(key)
...
func WriteFastTrieProgress(db DatabaseWriter, count uint64, appid ...string) {
	...
	if err := db.Put(key, new(big.Int).SetUint64(count).Bytes()); err != nil {
...
func ReadHeaderRLP(db DatabaseReader, hash common.Hash, number uint64, appid ...string) rlp.RawValue {
	var key = append(append(headerPrefix, encodeBlockNumber(number)...), hash.Bytes()...)
	if len(appid) == 1 && appid[0] != "" {
		key = append([]byte(appid[0]), key...)
	}
	data, _ := db.Get(key)
...
func HasHeader(db DatabaseReader, hash common.Hash, number uint64, appid ...string) bool {
	var key = append(append(append(headerPrefix, encodeBlockNumber(number)...), hash.Bytes()...))
	if len(appid) == 1 && appid[0] != "" {
		key = append([]byte(appid[0]), key...)
	}
...
func ReadHeader(db DatabaseReader, hash common.Hash, number uint64, appid ...string) *types.Header {
	...
	data := ReadHeaderRLP(db, hash, number, key)
	if len(data) == 0 {
		return nil
	}
...
func ReadBodyRLP(db DatabaseReader, hash common.Hash, number uint64, appid ...string) rlp.RawValue {
	var key = append(append(blockBodyPrefix, encodeBlockNumber(number)...), hash.Bytes()...)
	data, _ := db.Get(key)
	return data
}
...
func ReadTd(db DatabaseReader, hash common.Hash, number uint64, appid ...string) *big.Int {
	key := append(append(append(headerPrefix, encodeBlockNumber(number)...), hash[:]...), headerTDSuffix...)
	if len(appid) == 1 && appid[0] != "" {
		key = append([]byte(appid[0]), key...)
	}
	data, _ := db.Get(key)
...
func WriteTd(db DatabaseWriter, hash common.Hash, number uint64, td *big.Int, appid ...string) {
...
func ReadReceipts(db DatabaseReader, hash common.Hash, number uint64, appid ...string) types.Receipts {
	key := append(append(blockReceiptsPrefix, encodeBlockNumber(number)...), hash[:]...)
...
func ReadBlock(db DatabaseReader, hash common.Hash, number uint64, appid ...string) *types.Block {
	...
	header := ReadHeader(db, hash, number, key)
	if header == nil {
		return nil
	}
	body := ReadBody(db, hash, number, key)
	if body == nil {
		return nil
	}
...
func WriteBlock(db DatabaseWriter, block *types.Block, appid ...string) {
	...
	WriteBody(db, block.Hash(), block.NumberU64(), block.Body(), Key)
	WriteHeader(db, block.Header(), Key)
}
...
func DeleteBlock(db DatabaseDeleter, hash common.Hash, number uint64, appid ...string) {
	...
	DeleteReceipts(db, hash, number, key)
	DeleteHeader(db, hash, number, key)
	DeleteBody(db, hash, number, key)
	DeleteTd(db, hash, number, key)
}
...
func FindCommonAncestor(db DatabaseReader, a, b *types.Header, appid ...string) *types.Header {
	...
	for bn := b.Number.Uint64(); a.Number.Uint64() > bn; {
		a = ReadHeader(db, a.ParentHash, a.Number.Uint64()-1, key)
		if a == nil {
			return nil
		}
	}
	for an := a.Number.Uint64(); an < b.Number.Uint64(); {
		b = ReadHeader(db, b.ParentHash, b.Number.Uint64()-1, key)
		if b == nil {
			return nil
		}
	}
