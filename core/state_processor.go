...
func (p *StateProcessor) Process(block *types.Block, statedb *state.StateDB, cfg vm.Config) (types.Receipts, []*types.Log, uint64, error) {
	...
	// 完成block，应用任何特定于consensus engine的额外功能(例如块奖励)
	p.engine.Finalize(p.bc, header, statedb, block.Transactions(), block.Uncles(), receipts)
	return receipts, allLogs, *usedGas, nil
}

// ApplyTransaction尝试将事务应用于给定的状态数据库，并将输入参数用于其环境。
// 它返回~~  事务的收据、
// 			使用的气体、
// 			事务失败时的错误
// 如果事物失败，表明块无效。
func ApplyTransaction(config *params.ChainConfig, bc *BlockChain, author *common.Address, gp *GasPool, statedb *state.StateDB, header *types.Header, tx *types.Transaction, usedGas *uint64, cfg vm.Config) (*types.Receipt, uint64, error) {
	...
	*usedGas += gas
	...
	receipt.GasUsed = gas
	// 如果创建了一个合约，在账单中存储合约地址并给出创建新链的信息
	if msg.To() == nil {
		receipt.ContractAddress = crypto.CreateAddress(vmenv.Context.Origin, tx.Nonce(), config.AppId)
	}
