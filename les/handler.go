...
func (pc *peerConnection) Head(string) (common.Hash, *big.Int) {
	return pc.peer.HeadAndTd()
}

func (pc *peerConnection) RequestHeadersByHash(origin common.Hash, amount int, skip int, reverse bool, appId string) error {
	reqID := genReqID()
	rq := &distReq{
...
func (pc *peerConnection) RequestHeadersByNumber(origin uint64, amount int, skip int, reverse bool, appId string) error {
	reqID := genReqID()
	rq := &distReq{
		getCost: func(dp distPeer) uint64 {
			peer := dp.(*peer)
